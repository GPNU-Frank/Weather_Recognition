...
from models.triplet_loss1 import TripletLoss

from models import ContrastiveCenterLoss

# dataset
from dataset import MyData, MyData_2classes
import numpy as np
...
parser.add_argument('--train-path', default="G:\\vscode_workspace\\Weather_Recognition\\data_split_v6\\train\\")
parser.add_argument('--test-path', default="G:\\vscode_workspace\\Weather_Recognition\\data_split_v6\\test\\")
parser.add_argument('--imagesize', default=224, type=int)

parser.add_argument('--pretrained-path', default="G:\\vscode_workspace\\Weather_Recognition\\checkpoints\\mwd_resnet18_2classes\\05_10_10_36_fold_0_model_best.pth.tar")
# optimization options
parser.add_argument('--epochs', default=20, type=int, metavar='N',
                help='number of total epochs to run')
parser.add_argument('--start-epoch', default=0, type=int, metavar='N',
                help='manual epoch number (useful on restarts)')
parser.add_argument('--train_batch', default=16, type=int, metavar='N',
...
parser.add_argument('--triplet-margin', type=float, default=0.5, help="weight for center loss")
# parser.add_argument('--pretrained', default='pretrainedmodels/vgg_msceleb_resnet50_ft_weight.pkl', type=str, metavar='PATH', 
#                     help='path to latest checkpoint (default: none)')

# checkpoints
parser.add_argument('-c', '--checkpoint', default='checkpoints/my_data_v6_resnet18_contrastive_center_loss_transfer_480', type=str, metavar='PATH',
...
def main():
    ...
    val_dataset = MyData_2classes(root_path="G:\\vscode_workspace\\Weather_Recognition\\my_val_imgs_2\\", transform=valid_transform)
    ...
    # criterion_t = nn.TripletMarginLoss(margin=args.triplet_margin, p=2).cuda()
    # criterion_t = TripletLoss(margin=args.triplet_margin).cuda()
    criterion_c = ContrastiveCenterLoss(512, 2).cuda()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # train_iter = MyDataTriplet(root_path=args.train_path, transforms=train_transform, batch_size=args.train_batch, shuffle=True)
        train_iter = torch.utils.data.DataLoader(train_dataset, args.train_batch, shuffle=True)
        train_loss, train_acc = train(train_iter, model, criterion, criterion_c, optimizer, epoch, use_cuda)
...
def train(train_iter, model, criterion, criterion_c, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_iter):
        ...
        # compute output
            ...
            # loss_triplet = batch_hard_triplet_loss(targets, feat, args.triplet_margin)

            bz = targets.size(0)
            # if torch.equal(targets, torch.ones(bz).long().cuda()) or torch.equal(targets, torch.zeros(bz).long().cuda()):
            #     loss_triplet = 0
            # else:
            loss_contrastive = criterion_c(targets, feat)
            loss = per_loss + loss_contrastive
