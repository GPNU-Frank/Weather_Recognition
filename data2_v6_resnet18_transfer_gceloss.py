...
# model
from models import resnet50, resnet18, FocalLoss, TruncatedLoss
from models.label_smoothing import LabelSmoothing

# dataset
from dataset import MyData, MyDataGCE
import numpy as np
...
# checkpoints
parser.add_argument('-c', '--checkpoint', default='checkpoints/data2_resnet18_transfer_480_gce_loss', type=str, metavar='PATH',
...
def main():
    ...
    train_dataset = MyDataGCE(root_path=args.train_path, transform=train_transform)
    test_dataset = MyDataGCE(root_path=args.test_path, transform=valid_transform)
...
def train(train_iter, model, criterion, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets, indexes) in enumerate(train_iter):
        ...
        per_loss = criterion(per_outputs, targets, indexes)
...
def test(test_iter, model, criterion, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets, indexes) in enumerate(test_iter):
    # measure data loading time
        data_time.update(time.time() - end)
