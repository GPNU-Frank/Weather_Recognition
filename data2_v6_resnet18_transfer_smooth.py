...
# model
from models import resnet50, resnet18, FocalLoss, resnet18_smooth
from models.label_smoothing import LabelSmoothing
from models.my_smoothing_loss import MySmoothLoss
...
parser.add_argument('--train-path', default="G:\\vscode_workspace\\Weather_Recognition\\data2\\train\\")
parser.add_argument('--test-path', default="G:\\vscode_workspace\\Weather_Recognition\\data2\\test\\")
...
# checkpoints
parser.add_argument('-c', '--checkpoint', default='checkpoints/data2_resnet18_transfer_smooth_480', type=str, metavar='PATH',
...
def main():
    ...
    # model
    model = resnet18_smooth(pretrained=True)
...
def train(train_iter, model, criterion, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_iter):
        ...
        # compute output
        weights, per_outputs = model(inputs)
        # per_outputs = model(inputs)

        per_loss = criterion(per_outputs, targets, weights)
        ...
        # Rank Regularization
        if weights.size(0) == 8:
            _, top_idx = torch.topk(weights.squeeze(), 4)
            _, down_idx = torch.topk(weights.squeeze(), 4, largest = False)

            high_group = weights[top_idx]
            low_group = weights[down_idx]
            high_mean = torch.mean(high_group)
            low_mean = torch.mean(low_group)
            # diff  = margin_1 - (high_mean - low_mean)
            diff  = low_mean - high_mean + 0.2
