...
# model
from models import resnet50, resnet18, resnet18_centerloss

# dataset
from dataset import MyData, MyDataTriplet, MyData_2classes, MyDataNew
import numpy as np
...
parser.add_argument('--test-path', default="G:\\vscode_workspace\\Weather_Recognition\\val_imgs_3\\")
parser.add_argument('--train2-path', default='G:\\vscode_workspace\\Weather_Recognition\\data2\\train\\')
...
parser.add_argument('--train_batch', default=12, type=int, metavar='N',
                help='train batchsize')
parser.add_argument('--test-batch', default=12, type=int, metavar='N',
                help='test batchsize')
parser.add_argument('--mwd-batch', default=12, type=int, metavar='N',
                help='test batchsize')
...
parser.add_argument('--weight-domain', type=float, default=0.2, help="weight for domain adaptation loss")
parser.add_argument('--weight-my-data', type=float, default=0.8, help="weight for domain adaptation loss")
parser.add_argument('--weight-mwd', type=float, default=0.2, help="weight for domain adaptation loss")
# parser.add_argument('--pretrained', default='pretrainedmodels/vgg_msceleb_resnet50_ft_weight.pkl', type=str, metavar='PATH', 
#                     help='path to latest checkpoint (default: none)')

# checkpoints
parser.add_argument('-c', '--checkpoint', default='checkpoints/my_data_v6_resnet18_tripletloss_transfer_480_maml_5classes', type=str, metavar='PATH',
...
def main():
    ...
    # train_iter = MyData(root_path=args.train_path, transforms=train_transform, batch_size=args.train_batch, shuffle=True)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        mwd_dataset = MyData(root_path=args.train2_path, transform=valid_transform)
...
def train(train_iter, mwd_iter, model, criterion, criterion_t, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_iter):
        try:
            ...
            mwd_iter = iter(mwd_iter)
            mwd_inputs, mwd_targets = next(mwd_iter)
        mwd_inputs, mwd_targets = mwd_inputs.cuda(), mwd_targets.cuda()
        ...
        # compute output
        # get my data loss and grad
        feat, per_outputs = model(inputs)
        my_data_loss = criterion(per_outputs, targets)
        # grad = torch.autograd.grad(my_data_loss, model.parameters())
        # my_data_fast_weights = list(map(lambda p: p[1] - self.update_lr * p[0], zip(grad, self.net.parameters())))

        
        # get mwd data loss and grad
        mwd_feat, mwd_outputs = model(mwd_inputs)
        mwd_loss = criterion(mwd_outputs, mwd_targets)
        # grad = torch.autograd.grad(mwd_loss, model.parameters())
        # mwd_fast_weights = list(map(lambda p: p[1] - self.update_lr * p[0], zip(grad, self.net.parameters())))
        ...
        per_loss = args.weight_my_data * my_data_loss + args.weight_mwd * mwd_loss
