...
parser.add_argument('--triplet-margin', type=float, default=1.0, help="weight for center loss")
# parser.add_argument('--pretrained', default='pretrainedmodels/vgg_msceleb_resnet50_ft_weight.pkl', type=str, metavar='PATH', 
#                     help='path to latest checkpoint (default: none)')

# checkpoints
parser.add_argument('-c', '--checkpoint', default='checkpoints/my_data_v6_resnet18_tripletloss_bh_transfer_480', type=str, metavar='PATH',
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # train_iter = MyDataTriplet(root_path=args.train_path, transforms=train_transform, batch_size=args.train_batch, shuffle=True)
        train_iter = torch.utils.data.DataLoader(train_dataset, args.train_batch, shuffle=True)
        train_loss, train_acc = train(train_iter, model, criterion, criterion_t, optimizer, epoch, use_cuda)
...
def train(train_iter, model, criterion, criterion_t, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_iter):
        ...
        # compute output
            ...
            if torch.equal(targets, torch.ones(bz).long().cuda()) or torch.equal(targets, torch.zeros(bz).long().cuda()):
                ...
                loss_triplet = criterion_t(feat, targets)
            loss = per_loss + loss_triplet
