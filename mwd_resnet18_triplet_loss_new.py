...
# checkpoints
parser.add_argument('-c', '--checkpoint', default='checkpoints/mwd_resnet18_480_triplet_loss_new', type=str, metavar='PATH',
...
def main():
    ...
    global criterion_t
    criterion_t = TripletLoss(margin=1.0)
    ...
    for f_num in range(args.folds):
        ...
        data_split = [[None, None] for i in range(6)]
        torch.utils.data.Subset
        for i in range(6):
            subset = torch.utils.data.Subset(dataset, range(i * 10000, (i + 1) * 10000))
            data_split[i][0], data_split[i][1] = torch.utils.data.random_split(subset, [5000, 5000])
            # print(len(data_split[i]))
        train_split, test_split = [i[0] for i in data_split], [i[1] for i in data_split]
        train_dataset = torch.utils.data.ConcatDataset(train_split)
        test_dataset = torch.utils.data.ConcatDataset(test_split)
...
def train(train_iter, model, criterion, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_iter):
        ...
        loss_t = criterion_t(feat, targets)
        loss += loss_t
...
def save_checkpoint(state, is_best, f_num, checkpoint='checkpoint', filename='checkpoint.pth.tar'):
    filepath = os.path.join(checkpoint, 'fold_' + str(f_num) + '_' + 'epoch_' + str(state['epoch']) + filename)  # 每次epoch都保存
