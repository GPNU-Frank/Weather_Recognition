...
# model
from models import resnet50, resnet18, resnet18_centerloss, ContrastiveLoss

# dataset
from dataset import MyData, MyDataTriplet, MyData_2classes
import numpy as np
...
parser.add_argument('--weight-contrast', type=float, default=0.1, help="weight for center loss")
...
# checkpoints
parser.add_argument('-c', '--checkpoint', default='checkpoints/my_data_v6_resnet18_contrastiveloss_transfer_480', type=str, metavar='PATH',
...
def main():
    ...
    criterion_c = ContrastiveLoss(margin=args.triplet_margin)
...
def train(train_iter, model, criterion, criterion_c, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_iter):
        ...
        bz = feat.size(0)
        feat_1 = feat[:bz//2, :]
        target_1 = targets[ :bz//2]
        feat_2 = feat[bz//2:, :]
        target_2 = targets[bz//2:]
        ...
        loss_contrastive = criterion_c(feat_1, feat_2, label)
        loss = per_loss + args.weight_contrast * loss_contrastive
