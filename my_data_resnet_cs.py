...
# model
from models import resnet50, resnet50_adv
...
parser.add_argument('--train-path', default="G:\\vscode_workspace\\Weather_Recognition\\data_split_v2\\train\\")
parser.add_argument('--test-path', default="G:\\vscode_workspace\\Weather_Recognition\\data_split_v2\\test\\")
...
# checkpoints
parser.add_argument('-c', '--checkpoint', default='checkpoints/my_data_resnet50_cs', type=str, metavar='PATH',
                help='path to save checkpoint (default:checkpoint)')
parser.add_argument('--resume', default='', type=str, metavar='PATH')

# architecture
parser.add_argument('--arch', '-a', metavar='ARCH', default='resnet50_adv')
...
def main():
    ...
    train_dataset = MyDataCS(root_path=args.train_path, transform=train_transform)
    test_dataset = MyDataCS(root_path=args.test_path, transform=valid_transform)
    ...
    # model
    model = resnet50_adv(pretrained=True)
    if use_cuda:
        model = model.cuda()
...
def train(train_iter, model, criterion, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets, constrast, hist) in enumerate(train_iter):
        ...
        # compute output
        per_outputs = model(inputs, constrast, hist)
...
def test(test_iter, model, criterion, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets, constrast, hist) in enumerate(test_iter):
    # measure data loading time
        data_time.update(time.time() - end)
