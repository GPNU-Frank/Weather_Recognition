...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=5):
        ...
        self.fc_cat = nn.Linear(512 + 256 + 128 + 64, num_classes)
    ...
    def forward(self, x):
        ...
        dx_3 = self.diconv3(x)
        dx_3 = self.avgpool(dx_3)
        dx_3 = dx_3.view(x.size(0), -1)
        ...
        fuse_x = torch.cat([x, dx_1, dx_2, dx_3], dim=1)

        x = self.fc_cat(fuse_x)
        # x = self.fc(x)
 
        return x
           

def resnet18_dilate(pretrained=False, **kwargs):
