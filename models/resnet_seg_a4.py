...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=5):
        ...
        # modified
        self.inplanes = 64
        self.seg_layer2 = self._make_layer(block, 128, layers[1], stride=2)
        self.seg_layer3 = self._make_layer(block, 256, layers[2], stride=2)
        self.seg_layer4 = self._make_layer(block, 512, layers[3], stride=2)
        ...
        self.fc = nn.Linear(512 * block.expansion + 512 * block.expansion, num_classes)
    ...
    def forward(self, x, x_seg):
        ...
        x_seg = self.seg_layer2(x_seg)
        x_seg = self.seg_layer3(x_seg)
        x_seg = self.seg_layer4(x_seg)
        x = self.layer4(x)
...
def resnet18_seg_a4(pretrained=False, **kwargs):
